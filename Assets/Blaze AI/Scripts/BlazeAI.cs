...
public class BlazeAI : MonoBehaviour 
{
    ...
    bool deathDollCallParam;
    GameObject deathDollEnemyParam;
    ...
    void DeathDollCall()
    {
        Death(deathDollCallParam, deathDollEnemyParam, true);
    }
    ...
    // kill the AI - either plays animation or ragdolls - last parameter is for system use (ignore it), use the first 2
    ...
    public void DeathDoll(float timeToRagdoll, bool callOthers = false, GameObject enemy = null)
    {
        // return if already dead or Blaze disabled
        if (state == State.death || !enabled) {
            Debug.Log("DeathDoll() can't be called when the AI is in death state or Blaze AI is disabled.");
            return;
        }
        ...
        deathDollCallParam = callOthers;
        deathDollEnemyParam = enemy;
